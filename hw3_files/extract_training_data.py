...
class FeatureExtractor(object):
    ...
    def get_input_representation(self, words, pos, state):
        ...
        words_list = words[:]
        stack_list = state.stack[:]
        buffer_list = state.buffer[:]
        input = []

        while len(stack_list) < 3:
          stack_list.insert(0, len(words))
        while len(buffer_list) < 3:
          buffer_list.insert(0, len(words))
  
        # Make sure words_list contains "NULL" symbol
        words_list.append("<NULL>")

        # Make the stack in input
        for i in range(-1,-4,-1):
          index = stack_list[i]
          word = words_list[index]
  
          if word in self.word_vocab:
            input.append(self.word_vocab[word])
          elif isinstance(word, str) and word.lower() in self.word_vocab:
            input.append(self.word_vocab[word.lower()])
          elif pos[index] == 'NNP':
            input.append(self.word_vocab['<NNP>'])
          elif pos[index] == 'CD':
            input.append(self.word_vocab['<CD>'])
          elif not index:
            input.append(self.word_vocab['<ROOT>'])
          else:
            input.append(self.word_vocab['<UNK>'])
        
        # Make the buffer in input
        for i in range(-1,-4,-1):
          index = buffer_list[i]
          word = words_list[index]
